...
def apply_settings(assi, device, pattern, count)
	...
	0.upto(count.to_i - 1) { |i|
		...
		puts command
		if $dummy == false
			system("Terminal --geometry=80x12 --hide-toolbar --hide-menubar --disable-server -T \"" + extract_lang_string("deleting") + " " + (i + 1).to_s + 
	...
	system("exo-open /tmp/dc3dd_errors_" + logfile + ".txt")
	assi.destroy
end
